...
export function exportPortfolioToCSV(assets, language = 'id') {
  ...
  [...assets.stocks, ...assets.crypto].forEach(a => {
    ...
    if (a.currency === 'USD' || a.type === 'crypto') {
      // Use US format for USD/crypto without dollar sign
      avgPrice = formatNumberUSD(a.avgPrice || 0, 2);
      porto = formatNumberUSD(a.porto || 0, 2);
      gain = formatNumberUSD(a.gain || 0, 2);
    } else {
      // Use Indonesian format for IDR/stocks
      avgPrice = formatNumber(a.avgPrice || 0, 2);
      porto = formatNumber(a.porto || 0, 2);
      gain = formatNumber(a.gain || 0, 2);
    }
