...
export function formatQuantity(number, forceDecimals = null) {
  ...
  let decimals = forceDecimals;

  if (decimals === null) {
    if (num >= 1) {
      // For whole numbers or numbers >= 1, check if there's a meaningful decimal part
      const decimalPart = num % 1;
      if (decimalPart === 0) {
        decimals = 0; // No decimals needed for whole numbers
      } else if (decimalPart < 0.01) {
        decimals = 8; // Very small decimal, show more precision
      } else {
        decimals = 2; // Normal decimal
      }
    } else {
      // For numbers < 1 (fractional shares, small crypto amounts)
      if (num < 0.0001) {
        decimals = 8; // Very small, show up to 8 decimals
      } else if (num < 0.01) {
        decimals = 6; // Small, show up to 6 decimals
      } else if (num < 0.1) {
        decimals = 4; // Medium small, show 4 decimals
      } else {
        decimals = 2; // Close to 1, show 2 decimals
      }
    }
  }
  ...
  // Remove trailing zeros after decimal point, but keep at least significant digits
