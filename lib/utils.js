...
// Helper function to normalize number input intelligently (supports copy-paste IDR & USD formats)
export function normalizeNumberInput(value) {
  if (value === null || value === undefined) return '';
  let normalized = String(value).trim();

  // Remove spaces and currency symbols
  normalized = normalized.replace(/[\sRp$ۣ]/gi, '');

  if (normalized.includes('.') && normalized.includes(',')) {
    // Both exist. Check which one is the decimal separator (the last one)
    const lastDot = normalized.lastIndexOf('.');
    const lastComma = normalized.lastIndexOf(',');
    if (lastComma > lastDot) {
      // e.g. "1.000.000,50" -> IDR format. Remove dots, change comma to dot.
      normalized = normalized.replace(/\./g, '').replace(',', '.');
    } else {
      // e.g. "1,000,000.50" -> US format. Remove commas.
      normalized = normalized.replace(/,/g, '');
    }
  } else if (normalized.includes(',')) {
    // Only commas. e.g. "1,000,000" or "1,5"
    if (normalized.split(',').length > 2) {
      normalized = normalized.replace(/,/g, ''); // multiple commas -> thousands separators
    } else {
      const parts = normalized.split(',');
      if (parts[1].length === 3 && parts[0] !== '0' && parts[0] !== '-0') {
        normalized = normalized.replace(',', ''); // e.g. "1,000" -> 1000
      } else {
        normalized = normalized.replace(',', '.'); // e.g. "1,5" -> 1.5
      }
    }
  } else if (normalized.includes('.')) {
    // Only dots. e.g. "1.000.000" or "1.5" or "0.005"
    if (normalized.split('.').length > 2) {
      normalized = normalized.replace(/\./g, ''); // multiple dots -> IDR thousands separators
    } else {
      const parts = normalized.split('.');
      if (parts[1].length === 3 && parts[0] !== '0' && parts[0] !== '-0') {
        normalized = normalized.replace(/\./g, ''); // e.g. "1.000" -> 1000
      }
      // Otherwise leave the dot as decimal (e.g. "1.5", "0.005")
    }
  }
...
export function formatUSD(amount, decimals = 2) {
  ...
  const isNegative = num < 0;
  const formatted = formatNumberUSD(Math.abs(num), decimals);
