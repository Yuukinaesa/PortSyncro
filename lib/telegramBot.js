...
export async function sendDailyReports() {
  ...
  try {
    ...
    for (let i = 0; i < allUsers.length; i++) {
      ...
      try {
        // ALWAYS Autosnapshot everyone, every day.
        const { assets, prices, exchangeRate } = await getFullPortfolio(uid);
        await saveHistorySnapshot(uid, assets, prices, exchangeRate);
        snapped++;

        // If they opted into Telegram Autopilot, send them the message
        if (autopilotData && autopilotData.chatId) {
          if (autopilotData.lastSentAt && autopilotData.lastSentAt.startsWith(today)) {
            console.log(`[Cron] Skipping msg config ${uid}, already sent today`);
          } else {
            console.log(`[Cron] Sending daily report to chatId ${autopilotData.chatId} for UID ${uid}`);
            const recapText = fmt.portoRecap(assets, exchangeRate, prices);
        ...
        // Fast stagger: 500ms between users (allows ~100 users per 60s Vercel execution window)
        // Telegram limit is 30 msg/sec, 500ms (= 2 msg/sec) is extremely safe.
    ...
    console.log(`[Cron] Complete: ${snapped} snapped, ${sent} msgs sent, ${failed} failed`);
    return { success: true, snapped, sent, failed, totalUsers: allUsers.length };
  } catch (error) {
