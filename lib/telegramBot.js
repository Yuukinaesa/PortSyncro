...
async function getUserAssetsFallback(uid) {
  ...
  try {
    ...
    console.warn(`[TelegramBot] Fallback: rebuilt ${assets.stocks?.length || 0} stocks, ${assets.crypto?.length || 0} crypto from ${transactions.length} transactions (NO live prices)`);
...
async function getFullPortfolio(uid, { timeout = VERCEL_SAFE_TIMEOUT } = {}) {
  ...
  if (livePortfolio) {
    console.log(`[TelegramBot] Live portfolio: ${livePortfolio.assets?.stocks?.length || 0} stocks, ${livePortfolio.assets?.crypto?.length || 0} crypto`);
    return livePortfolio;
  }

  // Fallback: Rebuild from transactions (NOT stale history snapshots)
  console.warn('[TelegramBot] Live portfolio fetch timed out, rebuilding from transactions...');
  const cached = await getUserAssetsFallback(uid);
  if (cached) {
    console.warn(`[TelegramBot] Fallback portfolio: ${cached.assets?.stocks?.length || 0} stocks, ${cached.assets?.crypto?.length || 0} crypto (isCached=${cached.isCached})`);
    return cached;
  }

  // Ultimate fallback
  console.error('[TelegramBot] All portfolio fetch methods failed for UID:', uid);
...
async function handleDeleteTx(chatId, uid, txId) {
  ...
  try {
    ...
    console.error('[TelegramBot] handleDeleteTx error:', error);
...
async function handleEditPrompt(chatId, uid, txId) {
  ...
  try {
    ...
    console.error('[TelegramBot] handleEditPrompt error:', error);
...
async function handleEditTx(chatId, uid, args) {
  ...
  try {
    ...
    console.error('[TelegramBot] handleEditTx error:', error);
...
async function handlePrices(chatId, uid) {
  try {
    ...
    console.error('[TelegramBot] handlePrices error:', error);
  ...
  try {
    ...
    if (!lastSnapshotQuery.empty) {
      ...
      if (lastValue > 0 && totalValueIDR > 0) {
        ...
        if (dropPercent > 50) {
          console.warn(`[Snapshot] REJECTED for ${uid}: Value dropped ${dropPercent.toFixed(1)}% (${lastValue} → ${totalValueIDR}). Likely API failure.`);
          return; // Do NOT save this corrupted snapshot
        }
      }
    }
  } catch (checkErr) {
    console.warn('[Snapshot] Sanity check failed, proceeding with save:', checkErr.message);
  }
