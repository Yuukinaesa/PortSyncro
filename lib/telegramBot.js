...
export async function handleUpdate(update) {
  ...
  try {
    switch (commandPart) {
      ...
      case '/unlink': return withAuth(chatId, () => handleUnlink(chatId));
      case '/status': return handleStatus(chatId);
      case '/p':
      case '/portfolio': return withAuth(chatId, (uid) => handlePortfolio(chatId, uid));
      case '/stocks':
      case '/saham': return withAuth(chatId, (uid) => handleStocks(chatId, uid));
      case '/crypto': return withAuth(chatId, (uid) => handleCrypto(chatId, uid));
      case '/gold':
      case '/emas': return withAuth(chatId, (uid) => handleGold(chatId, uid));
      case '/cash': return withAuth(chatId, (uid) => handleCash(chatId, uid));
      case '/buy':
      case '/beli': return withAuth(chatId, (uid) => handleBuy(chatId, uid, args));
      case '/sell':
      case '/jual': return withAuth(chatId, (uid) => handleSell(chatId, uid, args));
      case '/addcash': return withAuth(chatId, (uid) => handleAddCash(chatId, uid, args));
      case '/addgold': return withAuth(chatId, (uid) => handleAddGold(chatId, uid, args));
      case '/price':
      case '/harga': return handlePrice(chatId, args);
      case '/history':
      case '/riwayat': return withAuth(chatId, (uid) => handleHistory(chatId, uid));
      case '/pnl': return withAuth(chatId, (uid) => handlePnL(chatId, uid));
      case '/porto': return withAuth(chatId, (uid) => handlePorto(chatId, uid));
      case '/autopilot': return withAuth(chatId, (uid) => handleAutopilot(chatId, uid, args));
      default:
...
async function handlePorto(chatId, uid) {
  ...
  await sendMessage(chatId, recapText, { parseMode: undefined }); // Plain text, no HTML
...
async function handleAutopilot(chatId, uid, args) {
  if (!adminDb) return sendMessage(chatId, fmt.errorMessage('Database not available'));

  const action = args[0]?.toLowerCase();

  if (action === 'on') {
    await adminDb.collection('telegram_autopilot').doc(String(chatId)).set({
      uid,
      chatId,
      enabled: true,
  ...
  if (action === 'off') {
    ...
    return sendMessage(chatId, `⏹ <b>Autopilot NONAKTIF</b>\n\nRekap harian sudah dimatikan.\nGunakan <code>/autopilot on</code> untuk mengaktifkan kembali.`);
  }

  // Show current status
  try {
    const doc = await adminDb.collection('telegram_autopilot').doc(String(chatId)).get();
...
<i>💡 Setiap user dijeda 1 menit untuk menghindari rate limit.</i>
<i>📊 Data juga otomatis ter-record ke Reports walaupun tidak buka web!</i>
    `.trim());
  } catch {
    return sendMessage(chatId, fmt.errorMessage('Gagal memuat status autopilot'));
  }
}
...
export async function sendDailyReports() {
  ...
  try {
    const snapshot = await adminDb.collection('telegram_autopilot')
      .where('enabled', '==', true)
      .get();

    if (snapshot.empty) {
      console.log('[TelegramBot] sendDailyReports: No users with autopilot enabled');
      return { success: true, sent: 0 };
    }

    const users = snapshot.docs.map(doc => doc.data());
    console.log(`[TelegramBot] sendDailyReports: ${users.length} users to send`);
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      try {
        ...
        if (user.lastSentAt && user.lastSentAt.startsWith(today)) {
          console.log(`[TelegramBot] Skipping ${user.chatId}, already sent today`);
          continue;
        }

        // Stagger: wait 5 seconds between users (except first)
        // 5s is safe for Telegram's 30 msg/sec limit and fits Vercel Hobby 60s maxDuration
        if (i > 0) {
          await new Promise(resolve => setTimeout(resolve, 5000)); // 5 seconds
        }

        console.log(`[TelegramBot] Sending daily report to chatId ${user.chatId} (${i + 1}/${users.length})`);

        const { assets, prices, exchangeRate } = await getFullPortfolio(user.uid);
        const recapText = fmt.portoRecap(assets, exchangeRate, prices);

        await sendMessage(user.chatId, `📅 <b>Rekap Harian (Autopilot)</b>\n\n${recapText}`, { parseMode: undefined });

        // Save history snapshot
        await saveHistorySnapshot(user.uid, assets, prices, exchangeRate);

        // Update last sent timestamp
        await adminDb.collection('telegram_autopilot').doc(String(user.chatId)).update({
          // Save with Jakarta timezone format to reliably check 'today'
          lastSentAt: new Date().toLocaleString('en-CA', { timeZone: 'Asia/Jakarta' }).replace(', ', 'T')
        });

        sent++;
      } catch (error) {
        console.error(`[TelegramBot] Failed to send daily report to ${user.chatId}:`, error.message);
        failed++;
      }
    }

    console.log(`[TelegramBot] sendDailyReports complete: ${sent} sent, ${failed} failed`);
    return { success: true, sent, failed, total: users.length };
  } catch (error) {
    console.error('[TelegramBot] sendDailyReports error:', error);
