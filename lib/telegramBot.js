...
// Cron endpoint has maxDuration: 60s, but external services like cron-job.org timeout at 30s.
// 12s is a sweet spot: more than the 8s webhook limit, but safe for 30s total execution.
const CRON_SAFE_TIMEOUT = 12000;
...
/**
 * Fallback: Rebuild from transactions with zero prices
 * Used when live price fetch times out
 * IMPORTANT: We NEVER use cached history snapshots as they may contain stale/ghost assets.
 * Instead, we rebuild from transactions (source of truth) but with last-known or zero prices.
 */
async function getUserAssetsFallback(uid) {
  if (!adminDb) return null;
  try {
    // ALWAYS rebuild from transactions — they are the ONLY source of truth.
    // History snapshots can contain stale assets that were already sold/deleted.
    const transactions = await getAllUserTransactions(uid);
    
    if (transactions.length === 0) {
      // No transactions, try user doc as absolute last resort
    ...
    // Get exchange rate from most recent history or user doc
    ...
    try {
      ...
      if (!recentSnapshot.empty) {
        const histData = recentSnapshot.docs[0].data();
        exchangeRate = histData.exchangeRate || 16500;
      }
    } catch (e) { /* use default */ }

    // Rebuild assets from transactions with empty prices
    // This ensures we show ONLY assets that actually exist in the user's portfolio
    const assets = rebuildAssetsFromTransactions(transactions, {}, exchangeRate);

    secureLogger.warn(`[TelegramBot] Fallback: rebuilt ${assets.stocks?.length || 0} stocks, ${assets.crypto?.length || 0} crypto from ${transactions.length} transactions (NO live prices)`);
    ...
    secureLogger.error('[TelegramBot] getUserAssetsFallback error:', error);
    return null;
  }
}
...
export async function sendMessage(chatId, text, options = {}) {
  try {
    ...
    if (!result.ok) {
      secureLogger.error('[TelegramBot] sendMessage error:', result.description);
    }
    return result;
  } catch (error) {
    secureLogger.error('[TelegramBot] sendMessage fetch error:', error);
    return null;
  }
}
...
async function getLiveExchangeRate(uid) {
  try {
    ...
    if (result && result.rate && result.rate > 0) {
      secureLogger.log(`[TelegramBot] Live exchange rate: ${result.rate} (${result.source})`);
      return result.rate;
    }
  } catch (e) {
    secureLogger.warn('[TelegramBot] Live exchange rate fetch failed:', e.message);
  }
  ...
  if (uid && adminDb) {
    try {
      ...
      if (savedRate && savedRate > 0) {
        secureLogger.log(`[TelegramBot] Using Firestore exchange rate: ${savedRate}`);
        return savedRate;
      }
    } catch (e) { /* continue */ }
  }
  ...
  secureLogger.warn('[TelegramBot] Using hardcoded fallback exchange rate');
  return 16500;
}
 ...
 */
async function getLivePrices(assets, { timeout = 5000 } = {}) {
  const prices = {};
  ...
  try {
    ...
    // Fetch in parallel with individual timeouts (use provided timeout)
    const [stockPrices, cryptoPrices, goldPrices] = await Promise.allSettled([
      stockTickers.length > 0 ? withTimeout(fetchStockPrices(stockTickers), timeout, {}) : Promise.resolve({}),
      cryptoSymbols.length > 0 ? withTimeout(fetchCryptoPrices(cryptoSymbols), timeout, {}) : Promise.resolve({}),
      (assets?.gold || []).length > 0 ? withTimeout(fetchGoldPrices(), timeout, null) : Promise.resolve(null)
    ]);
    ...
    secureLogger.error('[TelegramBot] getLivePrices error:', error);
  }
...
async function getLinkedUid(chatId) {
  ...
  try {
    ...
    secureLogger.error('[TelegramBot] Error getting linked UID:', error);
    return null;
  }
}
...
async function linkAccount(chatId, code, username) {
  ...
  try {
    ...
    secureLogger.error('[TelegramBot] linkAccount error:', error);
...
async function unlinkAccount(chatId) {
  ...
  try {
    ...
    secureLogger.error('[TelegramBot] unlinkAccount error:', error);
    return false;
  }
}
...
/**
 * Normalize a Firestore Timestamp or various date formats to ISO string.
 * Dashboard uses serverTimestamp() which returns a Firestore Timestamp object,
 * NOT an ISO string. Without normalization, new Date(firestoreTimestamp) returns
 * Invalid Date, breaking chronological sorting in rebuildAssetsFromTransactions.
 */
function normalizeTimestamp(ts) {
  ...
  if (ts && typeof ts._seconds === 'number') return new Date(ts._seconds * 1000).toISOString();
 ...
 * CRITICAL: Normalizes Firestore Timestamp objects to ISO strings so that
 * rebuildAssetsFromTransactions can sort them correctly.
 */
  ...
  try {
    ...
    return snapshot.docs.map(doc => {
      ...
      return {
        id: doc.id,
        ...data,
        // Normalize timestamp to ISO string for consistent sorting
        timestamp: normalizeTimestamp(data.timestamp)
      };
    });
  } catch (error) {
    secureLogger.error('[TelegramBot] getAllUserTransactions error:', error);
    return [];
  }
}
...
async function getRecentTransactions(uid, limit = 10) {
  ...
  try {
    ...
    secureLogger.error('[TelegramBot] getRecentTransactions error:', error);
    return [];
  }
}
 ...
 */
async function getFullPortfolio(uid, { timeout = VERCEL_SAFE_TIMEOUT } = {}) {
  // Try live rebuild with timeout protection
  const livePortfolio = await withTimeout(
    _getFullPortfolioLive(uid, timeout),
    timeout - 500, // Leave 0.5s buffer for response
    null
  );

  if (livePortfolio) {
    secureLogger.log(`[TelegramBot] Live portfolio: ${livePortfolio.assets?.stocks?.length || 0} stocks, ${livePortfolio.assets?.crypto?.length || 0} crypto`);
    return livePortfolio;
  }

  // Fallback: Rebuild from transactions (NOT stale history snapshots)
  secureLogger.warn('[TelegramBot] Live portfolio fetch timed out, rebuilding from transactions...');
  const cached = await getUserAssetsFallback(uid);
  if (cached) {
    secureLogger.warn(`[TelegramBot] Fallback portfolio: ${cached.assets?.stocks?.length || 0} stocks, ${cached.assets?.crypto?.length || 0} crypto (isCached=${cached.isCached})`);
    return cached;
  }

  // Ultimate fallback
  secureLogger.error('[TelegramBot] All portfolio fetch methods failed for UID:', uid);
 ...
 */
async function _getFullPortfolioLive(uid, timeout) {
  ...
  // Step 4: Fetch live prices for those tickers (pass timeout)
  // Give it slightly less than the overall timeout so the function can still return gracefully
  const fetchTimeout = timeout ? timeout - 500 : 5000;
  const prices = await getLivePrices(prelimAssets, { timeout: fetchTimeout });
...
async function addTransaction(uid, transaction) {
  ...
  try {
    ...
    secureLogger.log(`[TelegramBot] Transaction added: ${docRef.id} for user ${uid}`);
    return docRef.id;
  } catch (error) {
    secureLogger.error('[TelegramBot] addTransaction error:', error);
    return null;
  }
}
...
export async function handleUpdate(update) {
  ...
  try {
    switch (commandPart) {
      ...
      case '/setcash': return withAuth(chatId, fromId, (uid) => handleSetCash(chatId, uid, args));
      ...
      case '/hargaporto': return withAuth(chatId, fromId, (uid) => handlePrices(chatId, uid));
      ...
      case '/hapus': return withAuth(chatId, fromId, (uid) => handleDeleteAsset(chatId, uid, args));
      case '/autopilot': return withAuth(chatId, fromId, (uid) => handleAutopilot(chatId, uid, args));
      default:
        // Handle dynamic /del_TXID and /edit_TXID commands
        if (commandPart.startsWith('/del_')) {
          const txId = commandPart.replace('/del_', '');
          return withAuth(chatId, fromId, (uid) => handleDeleteTx(chatId, uid, txId));
        }
        if (commandPart.startsWith('/edit_')) {
          const txId = commandPart.replace('/edit_', '');
          return withAuth(chatId, fromId, (uid) => handleEditPrompt(chatId, uid, txId));
        }
        if (commandPart === '/edit') {
           return withAuth(chatId, fromId, (uid) => handleEditTx(chatId, uid, args));
        }
    ...
    secureLogger.error(`[TelegramBot] Error handling command ${commandPart}:`, error);
...
async function handleStatus(chatId, fromId) {
  ...
  try {
    ...
    secureLogger.error('[TelegramBot] handleStatus error:', error);
...
→ Jual 0.1 BTC @$70.000
  ...
  if (assetType === 'stock' || assetType === 'saham') {
    ...
    const matchingStocks = stocks.filter(s => s.ticker === ticker && s.market === market);
    const totalCurrentShares = matchingStocks.reduce((sum, s) => sum + (s.market === 'US' ? s.lots : s.lots * 100), 0);

    if (totalCurrentShares === 0) {
      return sendMessage(chatId, `❌ Penjualan gagal. Anda tidak memiliki aset <b>${ticker}</b> di pasar <b>${market}</b>.`);
    }

    if (totalCurrentShares < shares) {
      const currentUnit = isUS ? 'share' : 'lot';
      const currentAmountFormatted = isUS ? totalCurrentShares : (totalCurrentShares / 100);
      return sendMessage(chatId, `❌ Penjualan gagal. Saldo kepemilikan <b>${ticker}</b> (${market}) Anda saat ini: <b>${currentAmountFormatted} ${currentUnit}</b>. Anda mencoba menjual <b>${amount} ${currentUnit}</b>.`);
    }
    ...
    // Validate holdings
    const cryptoList = assets?.crypto || [];
    const cryptoAsset = cryptoList.find(c => c.symbol === ticker);
    const totalCurrentAmount = cryptoAsset ? cryptoAsset.amount : 0;

    if (totalCurrentAmount === 0) {
      return sendMessage(chatId, `❌ Penjualan gagal. Anda tidak memiliki aset <b>${ticker}</b>.`);
    }

    if (totalCurrentAmount - amount < -1e-9) {
      return sendMessage(chatId, `❌ Penjualan gagal. Saldo kepemilikan <b>${ticker}</b> Anda saat ini: <b>${totalCurrentAmount}</b>. Anda mencoba menjual <b>${amount}</b>.`);
    }
...
→ Tambah Rp1.000.000 ke GoPay
  ...
  // Last argument is always the amount, everything before is the bank name
  const bankName = args.slice(0, -1).join(' ').toUpperCase();
  let rawAmount = args[args.length - 1].replace(/\./g, '').replace(',', '.');
...
async function handleSetCash(chatId, uid, args) {
  if (args.length < 2) {
    return sendMessage(chatId, '❌ Format salah.\nGunakan: <code>/setcash BANK JUMLAH</code>\nContoh: <code>/setcash MYBCA 1100000</code>\nContoh: <code>/setcash BLU BCA 1.225.000</code>');
  }

  // Last argument is always the amount, everything before is the bank name
  const bank = args.slice(0, -1).join(' ').toUpperCase();
  // Strip dots from the amount string so 1.100.000 becomes 1100000
  let rawAmountStr = args[args.length - 1].replace(/\./g, '');
  // If they use commas for decimals, change the first comma to a dot
  rawAmountStr = rawAmountStr.replace(',', '.');
  const targetAmount = parseFloat(rawAmountStr);

  if (isNaN(targetAmount) || targetAmount < 0) {
    return sendMessage(chatId, '❌ Jumlah saldo harus berupa angka bulat positif / nol.');
  }

  // 1. Fetch current cash balance
  const portfolio = await getFullPortfolio(uid);
  const currentAsset = portfolio.assets?.cash?.find(c => c.ticker === bank);
  const currentAmount = currentAsset ? parseFloat(currentAsset.amount) : 0;

  const diff = targetAmount - currentAmount;
  
  if (Math.abs(diff) < 0.01) {
    return sendMessage(chatId, `ℹ️ Saldo <b>${bank}</b> sudah bernilai sama (Rp${targetAmount.toLocaleString('id-ID')}), tidak ada perubahan disimpan.`);
  }
  ...
  const transaction = {
    ...
    totalValue: amount,
    valueIDR: amount,
    valueUSD: portfolio.exchangeRate && portfolio.exchangeRate > 0 ? amount / portfolio.exchangeRate : 0,
    timestamp: new Date().toISOString(),
    currency: 'IDR',
    date: new Date().toLocaleString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    status: 'completed',
    description: 'Manual Balance Correction (Set Cash)',
    source: 'telegram'
  };

  const txId = await addTransaction(uid, transaction);
  if (!txId) return sendMessage(chatId, fmt.errorMessage('Gagal mengupdate saldo bank.'));

  let msg = `💵 <b>Saldo Diperbarui!</b>\n`;
  msg += `━━━━━━━━━━━━━━━━━━━━\n\n`;
  msg += `🏦 <b>${bank}</b>\n`;
  msg += `   Saldo Baru: <b>Rp${targetAmount.toLocaleString('id-ID')}</b>\n`;
  msg += `   Koreksi: ${diff > 0 ? '+' : '-'}Rp${amount.toLocaleString('id-ID')}\n`;
...
// ═══════════════════════════════════════════════════════════════
// /hapus — Delete entire asset from portfolio
// ═══════════════════════════════════════════════════════════════

async function handleDeleteAsset(chatId, uid, args) {
  if (args.length < 2) {
    return sendAuthMessage(chatId, uid, `
🗑️ <b>Format Hapus Aset:</b>

<b>Hapus Saham:</b>
<code>/hapus stock BBCA</code>
→ Hapus semua posisi BBCA

<b>Hapus Crypto:</b>
<code>/hapus crypto BTC</code>
→ Hapus semua posisi BTC

<b>Hapus Emas:</b>
<code>/hapus gold GOLD-DIGITAL</code>
<code>/hapus gold GOLD-ANTAM</code>

<b>Hapus Cash:</b>
<code>/hapus cash BCA</code>
→ Hapus akun kas BCA

<i>⚠️ Aset akan dihapus permanen dari portofolio!</i>
    `.trim());
  }

  const assetType = args[0].toLowerCase();
  // Everything after the asset type is the ticker/symbol (supports multi-word names like "BLU BCA")
  const ticker = args.slice(1).join(' ').toUpperCase();

  // Validate asset type
  const validTypes = ['stock', 'saham', 'crypto', 'gold', 'emas', 'cash'];
  if (!validTypes.includes(assetType)) {
    return sendMessage(chatId, '❌ Tipe aset harus <b>stock</b>, <b>crypto</b>, <b>gold</b>, atau <b>cash</b>.');
  }

  // Normalize asset type
  let normalizedType = assetType;
  if (assetType === 'saham') normalizedType = 'stock';
  if (assetType === 'emas') normalizedType = 'gold';

  // Fetch current portfolio to verify asset exists
  const { assets } = await getFullPortfolio(uid);
  let existingAsset = null;
  let keyField = 'ticker';

  if (normalizedType === 'stock') {
    existingAsset = (assets?.stocks || []).find(s => s.ticker?.toUpperCase() === ticker);
  } else if (normalizedType === 'crypto') {
    existingAsset = (assets?.crypto || []).find(c => c.symbol?.toUpperCase() === ticker);
    keyField = 'symbol';
  } else if (normalizedType === 'gold') {
    existingAsset = (assets?.gold || []).find(g => g.ticker?.toUpperCase() === ticker);
  } else if (normalizedType === 'cash') {
    existingAsset = (assets?.cash || []).find(c => c.ticker?.toUpperCase() === ticker);
  }

  if (!existingAsset) {
    return sendMessage(chatId, `❌ Aset <b>${ticker}</b> (${normalizedType}) tidak ditemukan di portofolio.\n\nKetik /portfolio untuk melihat aset yang ada.`);
  }

  // Build delete transaction (same as dashboard's deleteAsset in portfolioStateManager)
  const deleteTransaction = {
    assetType: normalizedType,
    [keyField]: ticker,
    type: 'delete',
    amount: existingAsset.amount || existingAsset.lots || 0,
    price: existingAsset.currentPrice || existingAsset.avgPrice || 0,
    total: 0,
    timestamp: new Date().toISOString(),
    description: 'Asset deleted by user via Telegram',
    source: 'telegram',
    currency: existingAsset.currency || 'IDR',
    valueIDR: existingAsset.portoIDR || existingAsset.porto || 0,
    valueUSD: existingAsset.portoUSD || 0,
  };

  // Add broker/exchange info if available
  if (existingAsset.broker) deleteTransaction.broker = existingAsset.broker;
  if (existingAsset.exchange) deleteTransaction.exchange = existingAsset.exchange;

  const txId = await addTransaction(uid, deleteTransaction);
  if (!txId) return sendMessage(chatId, fmt.errorMessage('Gagal menghapus aset'));

  return sendMessage(chatId, `✅ <b>Aset Berhasil Dihapus!</b>\n━━━━━━━━━━━━━━━━━━━━\n\n🗑️ <b>${ticker}</b> telah dihapus dari portofolio.\n\n📱 <i>Otomatis sync ke web app!</i>\n⏰ ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })} WIB`);
}

// ═══════════════════════════════════════════════════════════════
// FITUR EDIT & DELETE TRANSACTIONS (via Telegram /history)
// ═══════════════════════════════════════════════════════════════

async function handleDeleteTx(chatId, uid, txId) {
  ...
  try {
    ...
    if (!doc.exists) {
       return sendMessage(chatId, '❌ Transaksi tidak ditemukan atau sudah dihapus.');
    }
    const data = doc.data();
    const ticker = data.ticker || data.symbol || 'Aset';
    
    await txRef.delete();
    return sendMessage(chatId, `✅ <b>Berhasil Dihapus!</b>\n\nTransaksi <b>${ticker}</b> telah dihapus dari portofolio.\nKetik /history untuk melihat riwayat terbaru.`);
  } catch (error) {
    secureLogger.error('[TelegramBot] handleDeleteTx error:', error);
    return sendMessage(chatId, fmt.errorMessage('Gagal menghapus transaksi.'));
  }
}

async function handleEditPrompt(chatId, uid, txId) {
  if (!adminDb) return sendAuthMessage(chatId, uid, fmt.errorMessage('Database not available'));
  
  try {
    const doc = await adminDb.collection('users').doc(uid).collection('transactions').doc(txId).get();
    if (!doc.exists) {
       return sendMessage(chatId, '❌ Transaksi tidak ditemukan atau sudah dihapus.');
    }
    const tx = doc.data();
    const ticker = tx.ticker || tx.symbol || 'Aset';
    
    // Reverse display amount logic from website (Lots vs Shares)
    let amountStr = tx.assetType === 'cash' ? tx.amount : tx.assetType === 'crypto' || tx.assetType === 'gold' ? tx.amount : tx.market === 'US' ? tx.amount : tx.amount / 100;
    
    let msg = `✏️ <b>Edit Transaksi ${ticker}</b>\n\n`;
    msg += `Salin (copy) dan edit angka pada pesan box di bawah ini, lalu kirim kembali ke bot:\n\n`;
    
    if (tx.assetType === 'cash') {
       msg += `<code>/edit ${txId} ${amountStr}</code>`;
    } else {
       msg += `<code>/edit ${txId} ${amountStr} ${tx.price}</code>`;
    }
    
    msg += `\n\n<i>Keterangan Format: /edit ID [JUMLAH_BARU] [HARGA_BARU]</i>`;
    return sendMessage(chatId, msg);
  } catch (error) {
    secureLogger.error('[TelegramBot] handleEditPrompt error:', error);
    return sendMessage(chatId, fmt.errorMessage('Gagal mengambil data transaksi.'));
  }
}

async function handleEditTx(chatId, uid, args) {
  if (args.length < 2) {
     return sendMessage(chatId, '❌ Format salah. Gunakan fungsi edit dari menu /history.');
  }
  
  const txId = args[0];
  const newAmount = parseFloat(args[1]);
  const newPrice = args[2] ? parseFloat(args[2]) : null;
  
  if (isNaN(newAmount) || newAmount <= 0) return sendMessage(chatId, '❌ Jumlah harus angka positif.');
  if (newPrice !== null && (isNaN(newPrice) || newPrice <= 0)) return sendMessage(chatId, '❌ Harga harus angka positif.');
  ...
  try {
    ...
    const doc = await txRef.get();
    if (!doc.exists) {
       return sendMessage(chatId, '❌ Transaksi tidak ditemukan.');
    }
    ...
    if (tx.assetType === 'cash') {
       ...
       updates.totalValue = newAmount;
       updates.valueIDR = newAmount;
    } else {
       if (!newPrice) return sendMessage(chatId, '❌ Harga (PRICE) diperlukan untuk tipe aset ini.');
       
       let finalAmount = newAmount;
       if (tx.assetType === 'stock' && tx.market !== 'US') {
          finalAmount = newAmount * 100; // lots to shares conversion
       }
       
       updates.amount = finalAmount;
       updates.price = newPrice;
       updates.totalValue = finalAmount * newPrice;
       
       // valueIDR and valueUSD scaling based on original ratios
       const oldRatioIDR = tx.valueIDR / tx.totalValue;
       const oldRatioUSD = tx.valueUSD / tx.totalValue;
       
       updates.valueIDR = updates.totalValue * (oldRatioIDR || (tx.currency === 'IDR' ? 1 : 16500));
       updates.valueUSD = updates.totalValue * (oldRatioUSD || (tx.currency === 'USD' ? 1 : 1/16500));
    }
    
    await txRef.update(updates);
    
    const ticker = tx.ticker || tx.symbol || 'Aset';
    return sendMessage(chatId, `✅ <b>Berhasil Diedit!</b>\n\nTransaksi <b>${ticker}</b> telah diperbarui.\nJumlah: ${newAmount}${newPrice ? `\nHarga: ${newPrice}` : ''}`);
  } catch (error) {
    secureLogger.error('[TelegramBot] handleEditTx error:', error);
    return sendMessage(chatId, fmt.errorMessage('Gagal mengedit transaksi.'));
  }
}
...
async function handleSetName(chatId, uid, args) {
  ...
  try {
    ...
    // Sanitize: strip HTML tags and escape special chars to prevent injection
    const sanitizedName = rawName.replace(/[<>&"']/g, '').trim();

    if (!sanitizedName || sanitizedName.length === 0) {
      return sendAuthMessage(chatId, uid, fmt.errorMessage('Nama tidak valid. Gunakan huruf dan angka saja.'));
    }
    ...
    await adminDb.collection('users').doc(uid).update({
      telegramDisplayName: sanitizedName
    });

    return sendMessage(chatId, `✅ Identitas berhasil diubah menjadi: <b>${sanitizedName}</b>.\n\nSistem akan menggunakan identitas ini sebagai header di grup alih-alih menampilkan email aslimu.`);
  } catch (error) {
    secureLogger.error('[TelegramBot] handleSetName error:', error);
...
<code>/price gold</code> — Harga emas hari ini
  ...
  try {
    ...
    secureLogger.error('[TelegramBot] Price fetch error:', error);
    return sendMessage(chatId, fmt.errorMessage('Gagal mengambil data harga'));
  }
}

async function handlePrices(chatId, uid) {
  try {
    const { assets, prices, exchangeRate } = await getFullPortfolio(uid);
    return sendMessage(chatId, fmt.portfolioPrices(assets, prices, exchangeRate));
  } catch (error) {
    secureLogger.error('[TelegramBot] handlePrices error:', error);
    return sendMessage(chatId, fmt.errorMessage('Gagal memuat harga portfolio'));
  }
}
...
async function handlePorto(chatId, uid) {
  ...
  try {
    ...
    secureLogger.log(`[TelegramBot] History snapshot saved for user ${uid}`);
  } catch (e) {
    secureLogger.warn(`[TelegramBot] Failed to save history snapshot:`, e.message);
  }
}

// ═══════════════════════════════════════════════════════════════
// /autopilot — Toggle daily auto-send setiap malam
// ═══════════════════════════════════════════════════════════════
...
async function saveHistorySnapshot(uid, assets, prices, exchangeRate) {
  ...
  // Use Jakarta timezone for date, but shift +2 hours to avoid Vercel cron midnight jitter
  const jitterSafeDate = new Date(Date.now() + 2 * 60 * 60 * 1000);
  const jakartaDate = jitterSafeDate.toLocaleDateString('en-CA', { timeZone: 'Asia/Jakarta' });
  ...
  // ── Sanity Check: Reject corrupted snapshots ──────────────────
  // If API prices failed to fetch, some assets get currentPrice=0,
  // causing a massive fake drop. We compare against the last snapshot
  // and REFUSE to overwrite if the new value dropped >50%.
  try {
    const lastSnapshotQuery = await adminDb.collection('users').doc(uid)
      .collection('history').orderBy('timestamp', 'desc').limit(1).get();
    
    if (!lastSnapshotQuery.empty) {
      const lastData = lastSnapshotQuery.docs[0].data();
      const lastValue = lastData.totalValueIDR || 0;
      
      // Only check if the previous snapshot had meaningful data
      if (lastValue > 0 && totalValueIDR > 0) {
        const dropPercent = ((lastValue - totalValueIDR) / lastValue) * 100;
        
        if (dropPercent > 50) {
          secureLogger.warn(`[Snapshot] REJECTED for ${uid}: Value dropped ${dropPercent.toFixed(1)}% (${lastValue} → ${totalValueIDR}). Likely API failure.`);
          return; // Do NOT save this corrupted snapshot
        }
      }
    }
  } catch (checkErr) {
    secureLogger.warn('[Snapshot] Sanity check failed, proceeding with save:', checkErr.message);
  }
...
export async function sendDailyReports() {
  if (!adminDb) {
    secureLogger.error('[TelegramBot] sendDailyReports: adminDb not available');
  ...
  try {
    ...
    const runDayUTC = new Date().toLocaleDateString('en-CA', { timeZone: 'UTC' });
    ...
    for (let i = 0; i < allUsers.length; i += CHUNK_SIZE) {
      ...
      await Promise.all(chunk.map(async (user) => {
        ...
        try {
          ...
          const { assets, prices, exchangeRate } = await getFullPortfolio(uid, { timeout: CRON_SAFE_TIMEOUT });
          await saveHistorySnapshot(uid, assets, prices, exchangeRate);
          snapped++;
          ...
          if (isNightRun && autopilotData && autopilotData.chatId) {
            const alreadySent = 
              (autopilotData.lastSentDayUTC === runDayUTC) || 
              (autopilotData.lastSentAt && autopilotData.lastSentAt.startsWith(today) && !autopilotData.lastSentDayUTC);

            if (alreadySent) {
              secureLogger.log(`[Cron] Skipping msg for ${uid}, already sent today`);
            } else {
              secureLogger.log(`[Cron] Sending Nightly report to chatId ${autopilotData.chatId} for UID ${uid}`);
              const recapText = fmt.portoRecap(assets, exchangeRate, prices);
              ...
              await adminDb.collection('telegram_autopilot').doc(uid).update({
                ...
                lastSentDayUTC: runDayUTC
              });
              sent++;
            }
          }
        } catch (error) {
          secureLogger.error(`[Cron] Failed processing for UID ${uid}:`, error.message);
          failed++;
        }
      }));
    }

    secureLogger.log(`[Cron] Complete: ${snapped} snapped, ${sent} msgs sent, ${failed} failed (Run: ${isNightRun ? 'Night/Message' : 'Day/Silent'})`);
    return { success: true, snapped, sent, failed, totalUsers: allUsers.length, mode: isNightRun ? 'night' : 'day' };
  } catch (error) {
    secureLogger.error('[Cron] sendDailyReports error:', error);
