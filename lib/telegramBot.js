...
export async function sendAuthMessage(chatId, uid, text, options = {}) {
  // Hanya ambil email dan tambahkan header JIKA dipanggil di Grup (chatId negatif)
  if (Number(chatId) < 0) {
    let email = 'Akun Anda';
    if (uid && adminDb) {
      try {
        const uDoc = await adminDb.collection('users').doc(uid).get();
        if (uDoc.exists) email = uDoc.data().email || email;
      } catch(e) {}
    }
    return sendMessage(chatId, `📧 <b>${email}</b>\n\n${text}`, options);
  }
  ...
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      try {
        ...
        await sendAuthMessage(user.chatId, user.uid, `📅 <b>Rekap Harian (Autopilot)</b>\n\n${recapText}`, { parseMode: undefined });
