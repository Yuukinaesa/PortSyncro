...
class PortfolioStateManager {
  ...
  buildAssetsFromTransactions(transactions, prices) {
    ...
    secureLogger.log(`Building assets from maps - Stocks: ${stocksMap.size}, Crypto: ${cryptoMap.size}`);
    ...
    const crypto = Array.from(cryptoMap.entries())
      .map(([symbol, txs]) => {
        secureLogger.log(`Processing crypto ${symbol} with ${txs.length} transactions:`, txs.map(tx => ({
          id: tx.id,
          type: tx.type,
          amount: tx.amount,
          price: tx.price,
    ...
    secureLogger.log(`Final portfolio - Stocks: ${stocks.length}, Crypto: ${crypto.length}`);
