...
class PortfolioStateManager {
  ...
  initialize(initialAssets, initialTransactions = []) {
    ...
    const defaultAssets = { stocks: [], crypto: [], gold: [], cash: [] };
    this.state.assets = {
  ...
  updatePrices(newPrices) {
    ...
    try {
      const normalizedNewPrices = {};
      for (const [key, val] of Object.entries(newPrices)) {
        if (key === 'gold') {
          normalizedNewPrices[key] = val;
        } else {
          normalizedNewPrices[key.toUpperCase()] = val;
        }
      }

      const hasChanges = Object.keys(normalizedNewPrices).some(key => {
        if (key === 'gold') {
          return JSON.stringify(this.state.prices[key]) !== JSON.stringify(normalizedNewPrices[key]);
        }
        return this.state.prices[key]?.price !== normalizedNewPrices[key]?.price;
      });

      if (!hasChanges) return;

      this.state.prices = { ...this.state.prices, ...normalizedNewPrices };
      this.state.lastUpdate = new Date().toISOString();
  ...
  // Internal: Process queued updates
  _processQueue() {
    if (this.batchUpdates.length > 0) {
      // Process next item
      const next = this.batchUpdates.shift(); // FIFO

      // We explicitly reset updateInProgress to false momentarily to allow the method calls 
      // to re-acquire the lock. Since we are in _processQueue, we 'own' the lock context,
      // but the methods guard themselves.
      this.updateInProgress = false;

      if (next.type === 'transactions') {
        this.updateTransactions(next.data);
      } else if (next.type === 'prices') {
        this.updatePrices(next.data);
      }
      // Note: Recursing via the called methods' finally block
    } else {
  ...
  buildAssetsFromTransactions(transactions, prices) {
    ...
    // Only include transactions that are NOT deleted from history
    // Deleting a transaction from history should also reverse its effect on the portfolio
    const validTransactions = transactions.filter(tx => tx.deletedFromHistory !== true);
    // secureLogger.log(`Processing ${validTransactions.length} valid transactions for portfolio calculation`);
    ...
    const stocks = Array.from(stocksMap.entries())
      .map(([key, txs]) => { // Key is "TICKER" or "TICKER|BROKER"
        ...
        const priceData = prices[`${tickerUpper}.JK`] || prices[tickerUpper];
  ...
  updatePortfolioValues() {
    ...
    if (!this.state.assets) {
      this.state.assets = { stocks: [], crypto: [], gold: [], cash: [] };
    }
    ...
    this.state.assets.stocks = this.state.assets.stocks.map(stock => {
      ...
      const priceData = this.state.prices[`${tickerUpper}.JK`] || this.state.prices[tickerUpper];
      // Check for manual price override
      let currentPrice = 0;
      ...
      if (currentPrice > 0) {
        // [FIX BUG-01+03] Differentiate between US stocks (lots = raw shares) and IDX stocks (lots = lots, 1 lot = 100 shares)
        const isUS = stock.market === 'US';
        const totalShares = isUS ? stock.lots : stock.lots * 100;
        const currentValue = currentPrice * totalShares;
        const costBasis = stock.avgPrice * totalShares;
        const gainPercentage = costBasis > 0 ? ((currentValue - costBasis) / costBasis) * 100 : 0;

        let gain, gainIDR, gainUSD, porto, portoIDR, portoUSD;

        if (isUS) {
          // US Stocks: price is in USD, so all calculations are USD-native
          gain = currentValue - costBasis;           // USD
          gainUSD = gain;
          gainIDR = this.state.exchangeRate && this.state.exchangeRate > 0 ? Math.round(gain * this.state.exchangeRate) : 0;
          porto = currentValue;                      // USD
          ...
          // IDX Stocks: price is in IDR, so all calculations are IDR-native
          gain = currentValue - costBasis;           // IDR
          gainIDR = gain;
          gainUSD = this.state.exchangeRate && this.state.exchangeRate > 0 ? Math.round((gain / this.state.exchangeRate) * 100) / 100 : 0;
          porto = currentValue;                      // IDR
    ...
    this.state.assets.crypto = this.state.assets.crypto.map(crypto => {
      ...
      const priceData = this.state.prices[symbolUpper];
      // Check for manual price override
      let currentPrice = 0;
  ...
  // [FIX BUG-02] Directly modify state.transactions instead of pushing to batchUpdates
  // to avoid conflict with _processQueue() which uses the same batchUpdates array
  async addTransaction(transaction) {
    ...
    // Add to pending updates tracker
    this.state.pendingUpdates.add(transactionId);

    try {
      // Directly add to state transactions (avoids queue conflict with updateTransactions/_processQueue)
      const existingIdx = this.state.transactions.findIndex(t => t.id === transactionId);
      if (existingIdx < 0) {
      ...
      this.state.pendingUpdates.delete(transactionId);
    }
  }
  ...
  deleteAsset(assetType, symbol, additionalInfo = null) {
    // Determine the correct key field based on asset type
    // Gold uses 'ticker' (like stocks), not 'symbol'
    const keyField = (assetType === 'stock' || assetType === 'cash' || assetType === 'gold') ? 'ticker' : 'symbol';

    // Get existing asset data to store value info in delete transaction
    const existingAsset = this.getAsset(assetType, symbol);

    const deleteTransaction = {
      id: `delete_${Date.now()}`,
      assetType,
      [keyField]: symbol.toUpperCase(),
      type: 'delete',
    ...
    } else if (assetType === 'gold' && additionalInfo) {
      deleteTransaction.broker = additionalInfo;
    }
  ...
  getPortfolioSummary() {
    ...
    // [FIX ISSUE-06] Use portoIDR consistently to avoid mixing USD+IDR values in total
    const totalValue = stocks.reduce((sum, stock) => sum + (stock.portoIDR || 0), 0) +
      crypto.reduce((sum, crypto) => sum + (crypto.portoIDR || 0), 0) +
      gold.reduce((sum, g) => sum + (g.portoIDR || 0), 0) +
      cash.reduce((sum, c) => sum + (c.portoIDR || 0), 0);

    // [FIX ISSUE-06] Use totalCostIDR for cross-currency consistency
    const totalCost = stocks.reduce((sum, stock) => sum + (stock.totalCostIDR || stock.totalCost || 0), 0) +
      crypto.reduce((sum, crypto) => sum + (crypto.totalCostIDR || crypto.totalCost || 0), 0) +
