...
class PortfolioStateManager {
  ...
  buildAssetsFromTransactions(transactions, prices) {
    ...
    const gold = Array.from(goldMap.entries())
      .map(([key, txs]) => { // Key is "TICKER" or "TICKER|BROKER"
        ...
        // Determine current price based on subtype/brand
        if (subtype === 'digital') {
          ...
          if (goldPrices.physical && goldPrices.physical[b]) {
            ...
            // Fallback to digital sell price if specific physical price not found, or digital price
  ...
  updatePortfolioValues() {
    ...
    this.state.assets.gold = this.state.assets.gold.map(gold => {
      ...
      if (!gold.useManualPrice && !gold.isManual) {
        const subtype = gold.subtype || 'digital';
        const brand = (gold.brand || '').toLowerCase();

        if (subtype === 'digital') {
          currentPrice = goldPrices.digital?.sellPrice || goldPrices.digital?.price || currentPrice;
        } else {
          if (goldPrices.physical && goldPrices.physical[brand]) {
            currentPrice = goldPrices.physical[brand].price || currentPrice;
          } else {
            currentPrice = goldPrices.digital?.sellPrice || currentPrice;
          }
        }
      }
      ...
      if (currentPrice > 0) {
        ...
        return {
          ...
          gain,
          gainIDR: gain,
          gainUSD: this.state.exchangeRate ? gain / this.state.exchangeRate : 0,
