...
// ═══════════════════════════════════════════════════════════════
// HTML ESCAPE UTILITY (Prevents Telegram parse errors)
// ═══════════════════════════════════════════════════════════════
...
export function fmtDateShort(date) {
  const d = date instanceof Date ? date : new Date(date || Date.now());
  // Include time in short date format
  return d.toLocaleString('id-ID', {
    timeZone: 'Asia/Jakarta',
    day: 'numeric',
    month: 'short',
...
export function stockList(stocks, exchangeRate) {
  ...
  stocks.forEach((s, i) => {
    ...
    msg += `<b>${i + 1}. ${escapeHTML(s.ticker)}</b>`;
    if (s.broker) msg += ` <i>(${escapeHTML(s.broker)})</i>`;
    if (isUS) msg += ` 🇺🇸`;
    msg += `\n`;
...
export function cryptoList(cryptos, exchangeRate) {
  ...
  cryptos.forEach((c, i) => {
    ...
    msg += `<b>${i + 1}. ${escapeHTML(c.symbol)}</b>`;
    if (c.exchange) msg += ` <i>(${escapeHTML(c.exchange)})</i>`;
    msg += `\n`;
...
export function goldList(golds) {
  ...
  golds.forEach((g, i) => {
    ...
    msg += `<b>${i + 1}. ${escapeHTML(g.ticker || g.name)}</b> [${escapeHTML(typeLabel)}]\n`;
    if (g.broker) msg += `   Broker: ${escapeHTML(g.broker)}\n`;
...
export function cashList(cashes, exchangeRate) {
  ...
  let totalCash = 0;
  // Sort cash alphabetically
  const sortedCashes = [...cashes].sort((a, b) => (a.ticker || '').localeCompare(b.ticker || ''));
  
  sortedCashes.forEach((c, i) => {
    const amount = c.amount || 0;
    totalCash += amount;
    msg += `<b>${i + 1}. ${escapeHTML(c.ticker)}</b>\n`;
...
export function transactionConfirmation(type, details) {
  ...
  if (details.assetType === 'stock') {
    ...
    msg += `📈 <b>${escapeHTML(details.ticker)}</b> (${escapeHTML(details.market)})\n`;
    ...
    msg += `🪙 <b>${escapeHTML(details.symbol)}</b>\n`;
    ...
    msg += `🥇 <b>${escapeHTML(details.ticker)}</b>\n`;
...
export function priceInfo(ticker, priceData) {
  if (!priceData || !priceData.price) {
    return `❌ Harga untuk <b>${escapeHTML(ticker)}</b> tidak ditemukan.`;
  }

  const priceFmt = priceData.currency === 'USD' ? fmtUSD : fmtIDR;
  const emoji = gainEmoji(priceData.change || 0);

  let msg = `💹 <b>${escapeHTML(ticker.toUpperCase())}</b>\n`;
  msg += `━━━━━━━━━━━━━━━━━━━━\n\n`;
  ...
  msg += `📡 Sumber: ${escapeHTML(priceData.source || 'N/A')}\n`;
...
export function goldPriceInfo(goldPrices) {
  ...
  msg += `📡 Sumber: ${escapeHTML(goldPrices.source || 'Pegadaian')}\n`;
...
export function portfolioPrices(assets, prices, exchangeRate) {
  ...
  if (stocks.length > 0) {
    ...
    stockByTicker.forEach(({ ticker, market, positions }) => {
      ...
      msg += `• ${escapeHTML(ticker)}: ${priceFmt(currentPrice)}`;
  ...
  if (crypto.length > 0) {
    ...
    cryptoBySymbol.forEach(({ symbol, positions }) => {
      ...
      msg += `• ${escapeHTML(symbol)}: ${fmtUSD(currentPrice)}`;
      if (marketChange !== undefined && marketChange !== null) {
        msg += ` (${gainEmoji(marketChange)}${fmtPercent(marketChange)})`;
      }
      if (totalAmount > 0) {
  ...
  if (gold.length > 0) {
    ...
    if (goldPriceData.physical) {
      Object.entries(goldPriceData.physical).forEach(([brand, data]) => {
        if (data.price) {
          msg += `• ${escapeHTML(brand.toUpperCase())}: ${fmtIDR(data.price)}/g\n`;
        }
      });
    }
    msg += `\n`;
...
export function transactionHistory(transactions) {
  ...
  recent.forEach((tx, i) => {
    ...
    msg += `${typeEmoji} <b>${typeLabel}</b> ${escapeHTML(ticker)}`;
...
export function pnlSummary(assets, exchangeRate, prices) {
  ...
  if (stocks.length > 0) {
    ...
    if (sorted.length > 0) {
      msg += `   🏆 Best: ${escapeHTML(sorted[0].ticker)} ${fmtPercent(sorted[0].gainPercentage)}\n`;
      if (sorted.length > 1) {
        const bottom = sorted[sorted.length - 1];
        msg += `   🔻 Worst: ${escapeHTML(bottom.ticker)} ${fmtPercent(bottom.gainPercentage)}\n`;
      }
    }
    msg += `\n`;
  }
  ...
  if (crypto.length > 0) {
    ...
    if (sorted.length > 0) {
      msg += `   🏆 Best: ${escapeHTML(sorted[0].symbol)} ${fmtPercent(sorted[0].gainPercentage)}\n`;
      if (sorted.length > 1) {
        const bottom = sorted[sorted.length - 1];
        msg += `   🔻 Worst: ${escapeHTML(bottom.symbol)} ${fmtPercent(bottom.gainPercentage)}\n`;
      }
    }
    msg += `\n`;
  }
...
Telegram kamu telah sukses terhubung dengan akun PortSyncro:
...
export function portoRecap(assets, exchangeRate, prices) {
  ...
  text += `📦 Modal: ${fmtIDR(totals.totalCostNormIDR)}\n`;
  text += `${gainEmoji(totals.totalGainIDR)} P/L: ${fmtIDR(totals.totalGainIDR)} (${fmtPercent(totals.totalGainPct)})\n\n`;

  // ── Per-category summary with P/L ──
  ...
  if (assets?.stocks?.length) {
    ...
    (assets.stocks || []).forEach(s => {
      ...
      const costBasis = (parseFloat(s.avgPrice) || 0) * shareCount;
      stocksCostIDR += s.market === 'US' ? costBasis * safeRate : costBasis;
    });
    const stocksGainIDR = totals.totalStocksIDR - stocksCostIDR;
    const stocksGainPct = stocksCostIDR > 0 ? (stocksGainIDR / stocksCostIDR) * 100 : 0;
    text += `Saham: ${fmtIDR(totals.totalStocksIDR)} ${gainEmoji(stocksGainIDR)} ${fmtPercent(stocksGainPct)}\n`;
  }

  if (assets?.crypto?.length) {
    let cryptoCostIDR = 0;
    (assets.crypto || []).forEach(c => {
      const costBasis = (parseFloat(c.avgPrice) || 0) * (parseFloat(c.amount) || 0);
      cryptoCostIDR += costBasis * safeRate;
    });
    const cryptoGainIDR = totals.totalCryptoIDR - cryptoCostIDR;
    const cryptoGainPct = cryptoCostIDR > 0 ? (cryptoGainIDR / cryptoCostIDR) * 100 : 0;
    text += `Crypto: ${fmtIDR(totals.totalCryptoIDR)} ${gainEmoji(cryptoGainIDR)} ${fmtPercent(cryptoGainPct)}\n`;
  }

  if (assets?.gold?.length) {
    let goldCostIDR = 0;
    (assets.gold || []).forEach(g => {
      goldCostIDR += (parseFloat(g.avgPrice) || 0) * (parseFloat(g.amount) || 0);
    });
    const goldGainIDR = totals.totalGoldIDR - goldCostIDR;
    const goldGainPct = goldCostIDR > 0 ? (goldGainIDR / goldCostIDR) * 100 : 0;
    text += `Emas: ${fmtIDR(totals.totalGoldIDR)} ${gainEmoji(goldGainIDR)} ${fmtPercent(goldGainPct)}\n`;
  }
  ...
  // Helper: format per-holding P/L suffix
  const plSuffix = (asset) => {
    const gp = asset.gainPercentage;
    if (gp !== undefined && gp !== null && !isNaN(gp)) {
      return ` ${gainEmoji(gp)} ${fmtPercent(gp)}`;
    }
    return '';
  };
  ...
  if (assets?.cash?.length) {
    ...
    cashList.forEach(b => {
      text += `• ${escapeHTML((b.ticker || '').toUpperCase())}: ${fmtIDR(b.valueIDR)}\n`;
    });
    text += "\n";
  }
    ...
    sortedTickers.forEach(group => {
      ...
      if (sortedHoldings.length > 1) {
        ...
        sortedHoldings.forEach(stock => {
          ...
          text += `• ${escapeHTML(stock.broker || 'Manual')}: ${fmtIDR(stock.valueIDR)}${lotStr}${plSuffix(stock)}\n`;
        });
      } else {
        const stock = sortedHoldings[0];
        const lotDisplay = stock.market === 'US' ? fmtQuantity(stock.lots) : `${stock.lots} Lot`;
        const lotStr = stock.market === 'US' ? ` (${lotDisplay} Share)` : ` (${lotDisplay})`;
        text += `${escapeHTML(group.ticker)} — ${escapeHTML(stock.broker || 'Manual')}: ${fmtIDR(stock.valueIDR)}${lotStr}${plSuffix(stock)}\n`;
      }
    });
    text += "\n";
  }
  ...
  if (assets?.crypto?.length) {
    ...
    sortedCrypto.forEach(group => {
      ...
      if (sortedHoldings.length > 1) {
        text += `${escapeHTML(group.symbol)}\n`;
        sortedHoldings.forEach(crypto => {
          const unitDisplay = fmtQuantity(crypto.amount);
          text += `• ${escapeHTML(crypto.exchange || 'Manual')}: ${fmtIDR(crypto.valueIDR)} (${unitDisplay} Unit)${plSuffix(crypto)}\n`;
        });
      } else {
        const crypto = sortedHoldings[0];
        const unitDisplay = fmtQuantity(crypto.amount);
        text += `${escapeHTML(group.symbol)} — ${escapeHTML(crypto.exchange || 'Manual')}: ${fmtIDR(crypto.valueIDR)} (${unitDisplay} Unit)${plSuffix(crypto)}\n`;
      }
    });
    text += "\n";
  }
    ...
    sortedGold.forEach(group => {
      ...
      if (sortedHoldings.length > 1) {
        ...
        sortedHoldings.forEach(gold => {
          ...
          text += `• ${escapeHTML(label)}: ${fmtIDR(gold.valueIDR)} (${gramDisplay} gram)${plSuffix(gold)}\n`;
        });
      } else {
        ...
        const name = gold.broker || gold.exchange || gold.brand;
        const label = gold.subtype === 'digital' ? (name || 'Digital') : (name ? `${name} (Fisik/Batangan)` : '(Fisik/Batangan)');
        text += `${escapeHTML(group.ticker)} — ${escapeHTML(label)}: ${fmtIDR(gold.valueIDR)} (${gramDisplay} gram)${plSuffix(gold)}\n`;
      }
    });
  }
