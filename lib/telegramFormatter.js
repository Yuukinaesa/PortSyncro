...
<b>📈 Harga Real-time</b>
/prices - Harga aset dalam portomu
...
export function portfolioPrices(assets, prices, exchangeRate) {
  ...
  if (stocks.length > 0) {
    ...
    stocks.forEach(s => {
      ...
      let currentPrice = priceData?.price || s.currentPrice || s.avgPrice || 0;
      let change = s.gainPercentage;
      
      const priceFmt = s.market === 'US' ? fmtUSD : fmtIDR;
      msg += `• <b>${s.ticker}</b>: ${priceFmt(currentPrice)}`;
      if (change !== undefined && change !== null) {
        msg += ` (${gainEmoji(change)}${fmtPercent(change)})`;
      }
      msg += `\n`;
    });
    msg += `\n`;
  ...
  if (crypto.length > 0) {
    ...
    crypto.forEach(c => {
      ...
      let currentPrice = priceData?.price || c.currentPrice || c.avgPrice || 0;
      let change = c.gainPercentage;
      
      msg += `• <b>${c.symbol}</b>: ${fmtUSD(currentPrice)}`;
      if (change !== undefined && change !== null) {
        msg += ` (${gainEmoji(change)}${fmtPercent(change)})`;
      }
      msg += `\n`;
  ...
  if (gold.length > 0) {
    msg += `🥇 <b>Emas</b>\n`;
    const goldPrice = prices?.GOLD?.price || (gold.length > 0 ? gold[0].currentPrice : 0);
    const goldChange = gold.length > 0 ? gold[0].gainPercentage : null;
    msg += `• <b>Emas IDR</b>: ${fmtIDR(goldPrice)}/g`;
    if (goldChange !== undefined && goldChange !== null) {
      msg += ` (${gainEmoji(goldChange)}${fmtPercent(goldChange)})`;
    }
    msg += `\n\n`;
  }
