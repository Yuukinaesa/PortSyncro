      ...
      try {
        ...
        const googlePromise = (async () => {
          ...
          try {
            ...
            // Prev Close Scraping (Updated for Multi-Language Support)
            let prevClose = null;
            let prevText = '';

            // 1. Label search (English: "Previous close")
            ...
            // 2. Label search (Indonesian: "Penutupan sebelumnya") - FIX FOR LOCAL INDONESIAN IP
            if (!prevText) {
            ...
            // 3. Class search (Common Google Finance class for Key Stats value)
...
// --- EXCHANGE RATE (UPDATED FOR V6) ---
export async function fetchExchangeRate() {
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10000);

    // Support v6 API if key is present (e.g. in Vercel Env Vars), fallback to v4 free
