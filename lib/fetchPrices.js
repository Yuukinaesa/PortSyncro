...
export async function fetchIndoGoldPrices() {
  try {
    ...
    try {
      // 1. Try XAU (Spot Gold) - Preferred
      const globalData = await fetchCryptoPrices(['XAU']);
      let xauChange = (globalData && globalData['XAU']) ? globalData['XAU'].change : null;

      if (xauChange !== null && xauChange !== 0) {
        globalChange = xauChange;
        secureLogger.log(`Global Gold Change (XAU): ${globalChange}%`);
      } else {
        // 2. Try GC=F (Futures) if XAU is 0 or missing
        secureLogger.log(`XAU change is ${xauChange}, checking GC=F for activity...`);
        ...
        if (futuresChange !== null && futuresChange !== 0) {
          ...
          if (stockData && stockData['GLD'] && stockData['GLD'].change !== 0) {
            ...
            // All zero or failed, default to XAU result (even if 0)
            globalChange = xauChange !== null ? xauChange : 0;
