      ...
      try {
        ...
        const googlePromise = (async () => {
          try {
            ...
            // D. Currency Detection (Simple Heuristic for now)
            // If ticker ends with .JK, it's IDR. Otherwise, assume USD for Google Finance scrape.
...
export async function fetchCryptoPrices(symbols) {
  secureLogger.log('fetchCryptoPrices (Hybrid Binance + CryptoCompare) called with:', symbols);
  ...
  try {
    const binanceSymbols = uniqueSymbols.map(sym => {
    ...
    if (response.ok) {
      ...
      if (Array.isArray(data)) {
        data.forEach(item => {
          const matchedSym = uniqueSymbols.find(sym => 
    ...
    secureLogger.warn('Binance API fetch failed, falling back to CryptoCompare:', e);
  }

  // 2. Fallback to CryptoCompare for any missing symbols
  const missingSymbols = uniqueSymbols.filter(sym => !result[sym]);
  if (missingSymbols.length > 0) {
    try {
      const symbolsStr = missingSymbols.join(',');
      ...
      secureLogger.error('CryptoCompare fallback failed:', e);
    }
  }
