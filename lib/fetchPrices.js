  ...
  try {
    ...
    for (const ticker of tickers) {
      try {
        ...
        if (!response.ok) {
          secureLogger.warn(`Could not fetch data for ${ticker} from ${apiUrl} - status: ${response.status}`);
          continue;
        }
...
export async function fetchExchangeRate() {
  try {
    ...
    // This ensures money values are legible even if network is down or API fails
