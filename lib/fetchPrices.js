    ...
    for (const ticker of tickers) {
      try {
        ...
        if (exchange === 'JK' || (!exchange && normalizedSymbol.length <= 4 && normalizedSymbol === symbol.toUpperCase())) {
          ...
          // US Markets - use only the symbol, no suffix
          apiUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${normalizedSymbol}?interval=1d`;
          console.log(`Fetching US stock data for ${ticker} (symbol: ${normalizedSymbol}) from ${apiUrl}`);
        } else if (exchange === 'NASDAQ' || exchange === 'NYSE' || (!exchange && /^[A-Z]{1,5}$/.test(normalizedSymbol))) {
          apiUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${normalizedSymbol}?interval=1d`;
