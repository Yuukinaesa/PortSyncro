  ...
  if (missingSymbols.length > 0) {
    try {
      ...
      if (mappedIds.length > 0) {
        const cgUrl = `https://api.coingecko.com/api/v3/simple/price?ids=${mappedIds.join(',')}&vs_currencies=usd&include_24hr_change=true`;
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 5000);
        const response = await fetch(cgUrl, { signal: controller.signal, headers: { 'User-Agent': 'Mozilla/5.0' } });
        clearTimeout(timeoutId);
