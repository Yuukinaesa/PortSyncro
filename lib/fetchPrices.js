            ...
            if (res.ok) {
              ...
              if (priceText) {
                ...
                if (!isNaN(parsedPrice) && parsedPrice > 0) {
                  ...
                  // Usually in a div with classes like .JwB6zf (green) or .fe4pJf (red) or .P2Luy
                  // We construct a rough heuristic: look for percentage text in the same container
                  const pctText = priceElement.parent().parent().text();
                  const match = pctText.match(/([+-]?\d+(\.\d+)?)%/);
                  if (match) {
                    change = parseFloat(match[1]);
                    // Adjust sign if needed based on context (e.g. "-" symbol)
                    if (pctText.includes('-') && change > 0 && !match[1].includes('-')) {
                      change = -change;
                    }
                  }
