      ...
      try {
        ...
        if (exchange === 'JK' || (!exchange && normalizedSymbol.length <= 4 && normalizedSymbol === symbol.toUpperCase() && !/[0-9]/.test(normalizedSymbol) && normalizedSymbol !== 'GOOG' && normalizedSymbol !== 'GOOGL' && normalizedSymbol !== 'MSFT' && normalizedSymbol !== 'AAPL' && normalizedSymbol !== 'TSLA' && normalizedSymbol !== 'NVDA' && normalizedSymbol !== 'AMZN' && normalizedSymbol !== 'META')) {
          ...
          if (exchange === 'JK' || ticker.toUpperCase().endsWith('.JK')) {
            apiUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(normalizedSymbol)}.JK?interval=1m&range=1d&includePrePost=true`;
            secureLogger.log('Fetching IDX stock data from:', apiUrl);
          } else {
            // Treat as US/Global
            apiUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(normalizedSymbol)}?interval=1m&range=1d&includePrePost=true`;
        ...
        // Randomized User-Agents to avoid rate limiting
        const userAgents = [
        ...
        // Build symbol for Yahoo
        const yahooSymbol = (exchange === 'JK' || ticker.toUpperCase().endsWith('.JK'))
          ? `${normalizedSymbol}.JK`
          : normalizedSymbol;

        // Strategy 1: Try Yahoo Quote API (v7) - often has fresher data
        const quoteUrl = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(yahooSymbol)}&crumb=`;

        // Strategy 2: Fallback to Chart API (v8)
        const chartUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahooSymbol)}?interval=1m&range=1d&includePrePost=true`;

        secureLogger.log('Trying Quote API first:', quoteUrl);
        ...
        // Try Quote API first (v7)
        try {
          const quoteResponse = await fetchWithRetry(`${quoteUrl}&_=${timestamp}`, {
          ...
          if (quoteResponse.ok) {
            ...
            secureLogger.log('Quote API response for', ticker, ':', quoteData);
            ...
            if (quoteData.quoteResponse?.result?.[0]) {
              ...
              changeTime = '24h';
              secureLogger.log(`Quote API SUCCESS for ${ticker}: price=${price}, change=${change}%`);
            }
          }
        } catch (quoteError) {
          secureLogger.log('Quote API failed, trying Chart API:', quoteError.message);
        }

        // Fallback to Chart API (v8) if Quote failed
        if (!price) {
          secureLogger.log('Falling back to Chart API:', chartUrl);

          const chartResponse = await fetchWithRetry(`${chartUrl}&_=${timestamp}`, {
            signal: controller.signal,
            cache: 'no-store',
            headers: {
              'User-Agent': randomUA,
          ...
          secureLogger.log('Chart API response status:', chartResponse.status);

          if (!chartResponse.ok) {
            secureLogger.error(`Could not fetch data for ${ticker} - status: ${chartResponse.status}`);
            continue;
          }

          const data = await chartResponse.json();
          secureLogger.log('Chart API response data for', ticker, ':', data);

          if (data.chart?.result?.[0]?.meta?.regularMarketPrice) {
            price = data.chart.result[0].meta.regularMarketPrice;
            currency = data.chart.result[0].meta.currency || 'IDR';
        ...
        // Store result if price was found
        if (price) {
          result[ticker] = {
            price,
            currency,
          ...
          secureLogger.log('Successfully processed ticker:', ticker, 'with price:', price, 'change:', change.toFixed(2) + '%');
