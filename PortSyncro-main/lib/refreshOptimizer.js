...
class RefreshOptimizer {
  ...
  markRateLimitHit() {
    ...
    console.log('Rate limit hit, backing off for 30 seconds');
  }
  ...
  async triggerRefresh(refreshFunction) {
    console.log('Refresh optimizer triggered:', {
    ...
    if (this.canRefresh()) {
      ...
      // Queue the refresh
      console.log('Cannot refresh now, queuing request');
  ...
  queueRefresh(refreshFunction) {
    ...
    if (this.refreshQueue.length >= this.maxQueueSize) {
      console.log('Queue full, dropping oldest refresh request');
